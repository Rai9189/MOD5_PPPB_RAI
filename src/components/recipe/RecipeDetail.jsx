...
import { ArrowLeft, Heart, Clock, Users, ChefHat, Star, Edit, Trash2 } from 'lucide-react';
...
export default function RecipeDetail({ recipeId, onBack, onEdit, category = 'makanan' }) {
  ...
  if (recipeLoading) return <div className="min-h-screen flex items-center justify-center"><div className="text-center"><div className={`animate-spin rounded-full h-12 w-12 border-b-2 border-${colors.primary}-600 mx-auto`}></div><p className="mt-4 text-slate-600">Memuat resep...</p></div></div>;
  if (recipeError) return <div className="min-h-screen flex items-center justify-center p-4"><div className="text-center max-w-md"><div className="bg-red-50 border border-red-200 rounded-2xl p-6"><p className="text-red-600 font-semibold mb-2">Terjadi Kesalahan</p><p className="text-red-500 mb-4">{recipeError}</p><button onClick={onBack} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Kembali</button></div></div></div>;
  if (!recipe) return <div className="min-h-screen flex items-center justify-center p-4"><div className="text-center"><p className="text-slate-600">Resep tidak ditemukan</p><button onClick={onBack} className={`mt-4 px-4 py-2 bg-${colors.primary}-600 text-white rounded-lg hover:bg-${colors.primary}-700 transition-colors`}>Kembali</button></div></div>;
    ...
    <div className={`min-h-screen bg-gradient-to-br ${colors.gradient} pb-20 md:pb-8`}>
      ...
      <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 text-slate-700 hover:text-slate-900 transition-colors">
          ...
          <div className="flex items-center gap-2">
            {onEdit && (
              <>
                <button onClick={() => onEdit(recipeId)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl border border-white/40 mb-8">
          ...
          <div className="p-6 md:p-8">
            ...
            {recipe.description && <p className="text-slate-600 text-lg mb-6 leading-relaxed">{recipe.description}</p>}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white/70 backdrop-blur p-4 rounded-xl border border-white/60 text-center">
                ...
                <p className={`font-semibold capitalize ${getDifficultyColor(recipe.difficulty)}`}>{recipe.difficulty}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
            <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-3"><span>🥘</span> Bahan-bahan</h2>
            <ul className="space-y-3">
              {recipe.ingredients?.map((ingredient) => (
                <li key={ingredient.id} className="flex items-start gap-3 bg-white/50 p-3 rounded-xl border border-white/60">
          ...
          <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
            <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-3"><span>👨‍🍳</span> Langkah-langkah</h2>
            <ol className="space-y-4">
              {recipe.steps?.map((step) => (
                <li key={step.id} className="flex gap-4 bg-white/50 p-4 rounded-xl border border-white/60">
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full bg-${colors.primary}-600 text-white flex items-center justify-center font-bold text-sm`}>{step.step_number}</div>
        ...
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setShowReviewForm(!showReviewForm)} className={`px-4 py-2 bg-${colors.primary}-600 text-white rounded-xl hover:bg-${colors.primary}-700 transition-colors font-medium`}>
          ...
          {showReviewForm && (
            <form onSubmit={handleSubmitReview} className="mb-8 bg-white/70 rounded-2xl p-6 border border-white/60">
                ...
                <div className="flex gap-2">
                  {[1,2,3,4,5].map(star => (
                    <button key={star} type="button" onClick={() => setRating(star)} className="focus:outline-none">
                      <Star className={`w-8 h-8 ${star <= rating ? 'text-amber-500 fill-current' : 'text-slate-300'} hover:scale-110 transition-transform`} />
                    </button>
                  ))}
                </div>
              </div>
              <div className="mb-4">
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Bagikan pengalaman Anda dengan resep ini..." rows={4} className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500" maxLength={500} />
                <p className="text-sm text-slate-500 mt-1">{comment.length}/500 karakter</p>
              </div>
              <button type="submit" disabled={createLoading} className={`px-6 py-2 bg-${colors.primary}-600 text-white rounded-xl hover:bg-${colors.primary}-700 transition-colors font-medium`}>
          ...
          {reviewsLoading ? (
            ...
            <div className="space-y-4">
              {reviews.map(review => (
                <div key={review.id} className="border-b border-white/30 pb-4 last:border-b-0">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <p className="font-medium text-slate-800">{review.user_identifier}</p>
                      <p className="text-sm text-slate-500">{formatDate(review.created_at)}</p>
                    </div>
                    <div className="text-amber-400 flex gap-1">{getStarRating(review.rating)}</div>
                  </div>
